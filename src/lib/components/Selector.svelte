<script lang="ts">
	...
	export let handleChange: ({ key, value }: { key: string; value: SelectValue }) => void = () => {};
...
<Select.Root selected={$selection} onSelectedChange={(select) => handleChange({ key: label, value: select?.value })}>
